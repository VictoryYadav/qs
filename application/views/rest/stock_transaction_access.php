...
function getAvailableRoles(){
    ...
    $.post('<?= base_url('restaurant/stock_trans_access') ?>',{getAvailableRoles:1, UserId:UserId},function(res){
          ...
          if(data.length > 0){
            for (var i = 0; i < data.length; i++) {
                temp += `<div class="ck-button"  style="margin-left:-40px;">
                           <label>
                              <input type="checkbox" value="${data[i].TagId}" class="selectedAvailableRoles">&nbsp;&nbsp;<span>${data[i].Name}</span>
                           </label>
                        </div>`;
            }
          }else{
            temp = 'No Stock Transaction Found!';
          }
...
function getAssignedRoles(){
    var UserId = $(`#RUserId`).val();
    $.post('<?= base_url('restaurant/stock_trans_access') ?>',{getAssignedRoles:1, UserId:UserId},function(res){
        if(res.status == 'success'){
          var data = res.response;
          var temp = '';
          if(data.length > 0){
            for (var i = 0; i < data.length; i++) {
                temp += `<div class="ck-button"style="margin-left:-40px;">
                           <label>
                              <input type="checkbox" value="${data[i].TagId}" class="selectedAssignedRoles">&nbsp;&nbsp;<span>${data[i].Name}</span>
                           </label>
                        </div>`;
            }
          }else{
...
setRoles = () => {
    ...
    if(roleIds.length > 0){
        $.post('<?= base_url('restaurant/stock_trans_access') ?>',{setRestRoles:1, roles:roleIds, UserId:UserId},function(res){
            if(res.status == 'success'){
                alert(res.response);
                getUser();
            }else{
...
removeRoles = () => {
    ...
    if(cid.length > 0){
        $.post('<?= base_url('restaurant/stock_trans_access') ?>',{removeRestRoles:1, TagId:cid, UserId:UserId},function(res){
            if(res.status == 'success'){
                alert(res.response);
              getUser();
            }else{
