...
getAssignRestaurant = (userId) =>{
    $.post('<?= base_url('support/transfer_rest_to_user_alter') ?>',{type:'getRest', userId:userId},function(res){
...
setRoles = () => {
    ...
    if(eids.length > 0){
        if(olduserId != newUserId){
            $.post('<?= base_url('support/transfer_rest_to_user_alter') ?>',{type:'setRest', olduserId:olduserId, newUserId:newUserId, EID:eids},function(res){
                if(res.status == 'success'){
                    alert(res.response);
                    getUser();
                    newgetUser();
...
removeRoles = () => {
    ...
    if(eids.length > 0){
        $.post('<?= base_url('support/transfer_rest_to_user_alter') ?>',{type:'removeRest', olduserId:olduserId, newUserId:newUserId, EID:eids},function(res){
            if(res.status == 'success'){
                alert(res.response);
              // location.reload();
              getUser();
